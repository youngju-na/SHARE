...
base_command="python -m src.render \
  +experiment=dtu \
  data_loader.train.num_workers=0 \
  data_loader.test.num_workers=0 \
  data_loader.val.num_workers=0 \
  checkpointing.load=/home/youngju/ssd/ufosplat/checkpoints/recongs/reconGS-offset-disentangle-predcam-step_3000000.ckpt \
  mode=test \
