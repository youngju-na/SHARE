...
for view in "${views[@]}"; do
    if [[ $view -ne ${fixed_views[0]} && $view -ne ${fixed_views[1]} && $view -ne ${fixed_views[2]} ]]; then
        # Set the specific view combination and output path for this iteration
        test_ref_views="dataset.test_ref_views=[${fixed_views[0]},${fixed_views[1]},${fixed_views[2]},$view]"
        output_path="test.output_path=test/reconGS-any-views-4/${fixed_views[0]}-${fixed_views[1]}-${fixed_views[2]}-$view"

        # Construct and run the full command
        full_command="$base_command $test_ref_views $output_path"
        echo "Running command with views ${fixed_views[0]}, ${fixed_views[1]}, ${fixed_views[2]}, and $view"
        eval $full_command
    fi
done