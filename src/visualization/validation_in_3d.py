...
def render_projections(
    ...
    extra_label: str = "",
    all_gaussians: Gaussians | None = None,
    ...
    if all_gaussians is not None:
        all_minima = all_gaussians.means.min(dim=1).values
        all_maxima = all_gaussians.means.max(dim=1).values
        minima = torch.min(minima, all_minima)
        maxima = torch.max(maxima, all_maxima)
