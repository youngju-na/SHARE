...
def unproject_given_homogeneous(
...
def get_camera_rays(
    ...
    # In the camera coordinate system, the ray origin is always at (0, 0, 0).
    origins = torch.zeros_like(directions, device=directions.device)

    return origins, directions

def get_world_rays(
    coordinates: Float[Tensor, "*#batch dim"],
    extrinsics: Float[Tensor, "*#batch dim+2 dim+2"], #! cam to world
    intrinsics: Float[Tensor, "*#batch dim+1 dim+1"],
) -> tuple[
    Float[Tensor, "*batch dim+1"],  # origins
    Float[Tensor, "*batch dim+1"],  # directions
