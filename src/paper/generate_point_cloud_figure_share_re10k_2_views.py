...
# to prevent ImportError: attempted relative import with no known parent package
import sys
sys.path.append("src")
sys.path.append("src/paper")
...
#     ("fc60dbb610046c56", 28, 115, 10.0, [60, 70, 80, 90, 100, 110, 120], 3.0, 15),
#     ("1eca36ec55b88fe4", 0, 120, 10.0, [60, 70, 80, 90, 100, 110, 120], 3.0, 15), # teaser fig.
#     ("bc95e5c7e357f1b7", 35, 91, 10.0, [60, 70, 80, 90, 100, 110, 120], 3.0, 15),
#     ("bc95e5c7e357f1b7", 35, 91, 10.0, [60, 70, 80, 90, 100, 110, 120], 3.0, 15),
#     ("2c52d9d606a3ece2", 87, 112, 10.0, [60, 70, 80, 90, 100, 110, 120], 3.0, 15),
#     ("71a1121f817eb913", 139, 164, 10.0, [60, 70, 80, 90, 100, 110, 120], 3.0, 15),
#     ("d70fc3bef87bffc1", 67, 92, 10.0, [60, 70, 80, 90, 100, 110, 120], 3.0, 15),
#     ("f0feab036acd7195", 44, 69, 10.0, [60, 70, 80, 90, 100, 110, 120], 3.0, 15),
#     ("a93d9d0fd69071aa", 57, 82, 10.0, [60, 70, 80, 90, 100, 110, 120], 3.0, 15),
#     ("1bc87c160d1dc982", 12, 60, 10.0, [60, 70, 80, 90, 100, 110, 120], 3.0, 15),
# )

SCENES = (
    # scene, context 1, context 2, far plane
    ("fc60dbb610046c56", 28, 115, 10.0, [60, 80, 100, 120], 4.5, 12),
    ("1eca36ec55b88fe4", 70, 134, 10.0, [60, 80, 100, 120], 4.5, 12), # teaser fig.
    ("bc95e5c7e357f1b7", 35, 91, 10.0, [60, 80, 100, 120], 4.5, 12),
    ("2c52d9d606a3ece2", 28, 102, 10.0, [60, 80, 100, 120], 4.5, 12),
    ("71a1121f817eb913", 2, 71, 10.0, [60, 80, 100, 120], 4.5, 12),
    ("d70fc3bef87bffc1", 24, 89, 10.0, [60, 80, 100, 120], 4.5, 12),
    ("f0feab036acd7195", 14, 69, 10.0, [60, 80, 100, 120], 4.5, 12),
    ("a93d9d0fd69071aa", 34, 89, 10.0, [60, 80, 100, 120], 4.5, 12),
    ("1bc87c160d1dc982", 12, 60, 10.0, [60, 80, 100, 120], 4.5, 12),
)
...
GAUSSIAN_TRIM = 8
LINE_WIDTH = 10 
LINE_COLOR_PRED = [25/255., 240/255., 230/255.] # orange #b38000 # BGR? 255,92,0
...
def generate_point_cloud_figure(cfg_dict):
    ...
    for idx, (scene, *context_indices, far, angles, line_width, cam_div) in enumerate(
        ...
        view_sampler_cfg = ViewSamplerArbitraryCfg(
            ...
            target_views=[0, 0, 0],  # use [40, 80] for teaser
        )
        ...
        encoder_out = encoder.forward(
            example["context"], 0, False, visualization_dump=visualization_dump, scene_names=[scene], 
        )
        ...
        means = rearrange(
            gaussians.means, "() (v h w spp) xyz -> h w spp v xyz", v=1, h=h, w=w
        )
        opacities = rearrange(
            gaussians.opacities, "() (v h w spp) -> h w spp v", v=1, h=h, w=w
        )
        means = homogenize_points(means)
        w2c = example["context"]["extrinsics_gt"].inverse()[0][0:1]
        
        #! camera space
        means = einsum(w2c, means, "v i j, ... v j -> ... v i")[..., :3]
        shs = rearrange(
            gaussians.harmonics, "() (v h w spp) c sh -> h w spp v c sh", v=1, h=h, w=w
        ) 
        ...
        mask = mask & (means[..., 2] < far ) & (means[..., 2] > 0.1) & (opacities > 0.1)
        mask = mask.squeeze(-1)
        
        def trim(element):
            element = rearrange(
                element, "() (h w spp) ... -> h w spp ...", h=h, w=w
            )
        ...
        for angle in angles:
            ...
            rotation = R.from_euler("xyz", [-15, angle - 70, 0], True).as_matrix()
            ...
            translation[2, 3] = far * (0.5 ** (1 / 2))
            pose = translation @ pose

            ones = torch.ones((1,), dtype=torch.float32, device=device)
            render_args = {
                "extrinsics": example["context"]["extrinsics_gt"][0, :1] @ pose,
                "width": ones * far * 1, #! change to 2 for re10k, 1 for dtu
                "height": ones * far * 1, #! change to 2 for re10k, 1 for dtu
                "near": ones * 1, #! 0 for re10k, -1.0 for dtu
                "far": ones * far,
            ...
            frustum_corners = unproject_frustum_corners(
                torch.cat([example["context"]["extrinsics_gt"][0], encoder_out['pred_cam']['mats'][0]], dim=0),
                repeat(example["context"]["intrinsics"][0][0:1], "c i j -> (c v) i j", v=example["context"]["extrinsics_gt"].shape[1]*2),
                torch.ones((2*2,), dtype=torch.float32, device=device) * far / cam_div, #! view 개수
            )
            camera_origins = torch.cat([example["context"]["extrinsics_gt"][0, :, :3, 3], encoder_out['pred_cam']['mats'][0, :, :3, 3]], dim=0)
            ...
            *_, h, w = example["context"]["image"].shape
            ...
            # )
            c2w_mat = repeat(
                example["context"]["extrinsics_gt"][0, 0:1, :3, :3],
                "v a b -> h w spp v a b",
                h=224,
                w=224,
                spp=1,
            ...
            # ).as_matrix()
            # world_mat = c2w_mat.detach().cpu().numpy().squeeze() @ cam_rotations_np
            ...
            export_ply(
                example["context"]["extrinsics_gt"][0, 0],
                trim(gaussians.means)[0],
                trim(visualization_dump["scales"])[0],
                trim(gaussians.rotations)[0],
