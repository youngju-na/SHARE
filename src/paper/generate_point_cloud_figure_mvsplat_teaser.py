...
SCENES = [(x, *scene_cfgs[x]["context"], 6.0, [80], 1.5, 18) for x in scene_names]

# SCENES = (
#     # scene, context 1, context 2, far plane
#     # ("fc60dbb610046c56", 28, 115, 10.0),
#     ("1eca36ec55b88fe4", 0, 120, 10.0, [110], 1.5, 18),  # teaser fig.
#     ("2c52d9d606a3ece2", 87, 112, 35.0, [105], 1.5, 18),
#     ("71a1121f817eb913", 139, 164, 10.0, [65], 1.5, 18),
#     ("d70fc3bef87bffc1", 67, 92, 10.0, [60], 1.5, 18),
#     ("f0feab036acd7195", 44, 69, 25.0, [125], 1.5, 18),
#     ("a93d9d0fd69071aa", 57, 82, 15.0, [60], 1.5, 18),
#     ("572acd18419c3456", 0, 62, 6.0, [111], 1.5, 18),
#     ("3f79dc32d575bcdc", 30, 140, 10.0, [80], 1.5, 18),
#     ("2e2ad99d45033d6a", 178, 278, 10.0, [80], 1.5, 18),
#     ("7f6800a9878b31c7", 114, 180, 5.0, [80], 1.5, 18),
#     ("bc95e5c7e357f1b7", 35, 91, 5.0, [100], 1.5, 18),
#     ("84a2ee4663daf456", 31, 98, 12.0, [100], 1.5, 18),
#     ("df0389efcc51ac2d", 7, 83, 8.0, [100], 1.5, 18),
#     ("2fdfa70413053b84", 18, 106, 6.0, [100], 1.5, 18),
#     ("21d9134faec148f2", 99, 235, 10.0, [100], 1.5, 18),
#     ("f649244a6907838c", 15, 104, 6.0, [100], 1.5, 18),
#     ("f99691764cd67e0c", 49, 150, 6.0, [100], 1.5, 18),
#     ("be9fe7824449d416", 24, 137, 5.0, [100], 1.5, 18),
#     ("afe20f15c69bbb54", 0, 85, 6.0, [100], 1.5, 18),
#     ("0972074fece891f2", 0, 100, 8.0, [110], 1.5, 18),
#     ("fc6f664a700121e9", 72, 164, 5.0, [90], 1.5, 18),
# )
...
def generate_point_cloud_figure(cfg_dict):
    ...
    for idx, (scene, *context_indices, far, angles, line_width, cam_div) in enumerate(
        ...
        encoder_out = encoder.forward(
            example["context"], 0, False, scene_names=example['scene'], visualization_dump=visualization_dump
        )
        ...
        w2c = example["context"]["extrinsics"].inverse()[0][0:1]
        ...
        for angle in angles:
            ...
            *_, h, w = example["context"]["image"].shape
            ...
            # )
            c2w_mat = repeat(
                example["context"]["extrinsics"][0, 0:1, :3, :3],
                "v a b -> h w spp v a b",
                h=224,
                w=224,
                spp=3, #! num ofset 개수
            ...
            export_ply(
                ...
                trim(visualization_dump["rotations"])[0], #!world_rotations,
