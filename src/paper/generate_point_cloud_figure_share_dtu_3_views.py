...
SCENES = (
    ...
    # ("fc60dbb610046c56", 28, 115, 10.0),
    # ("1eca36ec55b88fe4", 0, 120, 3.0, [110]), # teaser fig.
    # ("bc95e5c7e357f1b7", 35, 91, 10.0, [60]),
    # ("bc95e5c7e357f1b7", 35, 91, 5.0, [60]),
...
SCENES = (
    # scene, context 1, context 2, far plane
    ("scan24", 25, 24, 33, 2.6, [50,100,150],  3.0, 15),
    ("scan37", 25, 24, 33, 2.6, [50,100,150],  3.0, 15),
    ("scan40", 25, 24, 33, 2.6, [50,100,150],  3.0, 15),
    ("scan55", 25, 24, 33, 2.6, [50,100,150],  3.0, 15),
    ("scan63", 25, 24, 33, 2.6, [50,100,150],  3.0, 15),
    ("scan65", 25, 24, 33, 2.6, [50,100,150],  3.0, 15),
    ("scan69", 25, 24, 33, 2.6, [50,100,150],  3.0, 15),
    ("scan83", 25, 24, 33, 2.6, [50,100,150],  3.0, 15),
    ("scan97", 25, 24, 33, 2.6, [50,100,150],  3.0, 15),
    ("scan105", 25, 24, 33, 2.6, [50,100,150], 3.0, 15),
    ("scan106", 25, 24, 33, 2.6, [50,100,150], 3.0, 15),
    ("scan110", 25, 24, 33, 2.6, [50,100,150], 3.0, 15),
    ("scan114", 25, 24, 33, 2.6, [50,100,150], 3.0, 15),
    ("scan118", 25, 24, 33, 2.6, [50,100,150], 3.0, 15),
    ("scan122", 25, 24, 33, 2.6, [50,100,150], 3.0, 15),
)
...
def generate_point_cloud_figure(cfg_dict):
    ...
    for idx, (scene, *context_indices, far, angles, line_width, cam_div) in enumerate(
        ...
        mask = mask & (means[..., 2] < far ) & (means[..., 2] > 1.0) & (opacities > 0.1)
        mask = mask.squeeze(-1)
        ...
        for angle in angles:
            ...
            frustum_corners = unproject_frustum_corners(
                ...
                torch.ones((3*2,), dtype=torch.float32, device=device) * far / cam_div, #! view 개수
            )
