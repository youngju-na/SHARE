...
@dataclass
class DatasetTanksAndTemplesCfg(DatasetCfgCommon):
    name: Literal['tanksandtemples']
    root_dir: str
    scan_id: list[str]
...
class DatasetTanksAndTemples(IterableDataset):
    cfg: DatasetTanksAndTemplesCfg
    stage: Stage
    ...
    def __init__(
        ...
        if self.cfg.name == "blendedmvs":
            ...
            self.img_wh = [960, 544]
        elif self.cfg.name == "tanks_and_temples":
            self.img_wh = [960, 540]
    ...
    def __iter__(self):
        ...
        for idx, meta in enumerate(self.metas):
            ...
            for i, vid in enumerate(view_ids):
                ...
                elif self.dataset == "tanksandtemples":
                    img_filename = os.path.join(self.data_dir, scan_id, 'images/{:0>8}.jpg'.format(vid))
        ...
        for scan_id in self.scan_ids:
            ...
            with open(pair_file) as f:
                ...
                for view_idx in range(num_viewpoint):
                    ref_view = int(f.readline().rstrip())
                    src_views = [int(x) for x in f.readline().rstrip().split()[1::2]]
                    src_views = [src_views[0]] + [src_views[-1]]
