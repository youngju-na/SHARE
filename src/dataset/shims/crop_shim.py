...
def apply_crop_shim_to_views(views: AnyViews, shape: tuple[int, int]) -> AnyViews:
    images, intrinsics = rescale_and_crop(views["image"], views["intrinsics"].detach(), shape)
    images_normalized, _ = rescale_and_crop(views["image_normalized"], views["intrinsics"].detach(), shape)
    
    intrinsics_org_scale = intrinsics.clone()
    intrinsics_org_scale[:, 0, 0] *= shape[1] 
    intrinsics_org_scale[:, 1, 1] *= shape[0]
    intrinsics_org_scale[:, 0, 2] *= (shape[1] )
    intrinsics_org_scale[:, 1, 2] *= (shape[0] )

    inv_K_org = intrinsics_org_scale.inverse()
    ...
    return {
        **views,
        "image": images,
        "image_normalized": images_normalized,
        "intrinsics": intrinsics,
        "intrinsics_org_scale": intrinsics_org_scale,   
        "inv_K_org": inv_K_org,
        "inv_K": inv_K,
    }
