    ...
    def __iter__(self):
        ...
        for idx, meta in enumerate(self.chunks):
            ...
            if self.stage == 'train':
                ...
                # Load the chunk.
                scan, light_idx, context_views, target_views = meta
                view_ids = context_views + target_views
                if len(context_views) > 0:
                    assert self.cfg.view_sampler.num_context_views == len(context_views), "Number of context views does not match the length of test context views"
                
                ref_view = context_views[0]
            ...
            for i, vid in enumerate(view_ids):
                ...
                # mask_filename = os.path.join(str(self.cfg.roots[0]), f'Masks/{scan}_train/mask_{vid:04d}.png')
                ...
                w2cs.append(self.all_extrinsics[index_mat]) #* reference view to source view
           ...
           #! make the first extrinsics to be the reference view (identical rotation, zero translation) by projection matrices
            ...
            example = {
                ...
                "target": {
                    ...
                    # "mask": target_masks, #* B x 3 x H x W
            ...
            for scan in self.scans:  #* test인 경우 15개의 scenes
                with open(self.pair_filepath) as f:
                    ...
                    if self.stage == "test":
                        ...
                        if len(self.cfg.test_context_views) > 0:
                        ...
                        # else:
                        #     context_views = [src_views[0], src_views[len(src_views) // 2], src_views[-1]]
                        #     target_views = [view for view in src_views if view not in context_views]
