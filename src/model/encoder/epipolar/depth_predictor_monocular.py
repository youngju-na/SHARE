...
class DepthPredictorMonocular(nn.Module):
    ...
    def forward(
        ...
        pdf = self.to_pdf(pdf_raw) #* softmax
        offset = self.to_offset(offset_raw) #* sigmoid

        # Sample from the depth distribution.
        index, pdf_i = self.sampler.sample(pdf, deterministic, gaussians_per_pixel) #* pdf_i is equivalent to the density
