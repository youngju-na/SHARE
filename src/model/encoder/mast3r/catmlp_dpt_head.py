...
def reg_gaussian(gaussian, mode):
    if mode is not None and 'norm' in mode:
        gaussian = gaussian / gaussian.norm(dim=-1, keepdim=True) 
    return gaussian


def postprocess(out, depth_mode, conf_mode, desc_dim=None, desc_mode='norm', two_confs=False, desc_conf_mode=None, gaussian_dim=None, gaussian_mode=None):
    ...
    if gaussian_dim is not None:
        start = 3 + int(conf_mode is not None) + desc_dim
        res['gaussian'] = reg_gaussian(fmap[..., start:start + gaussian_dim], mode=gaussian_mode)
    return res
...
class Cat_MLP_LocalFeatures_DPT_Pts3d(PixelwiseTaskWithDPT):
    ...
    def __init__(self, net, has_conf=False, local_feat_dim=16, hidden_dim_factor=4., hooks_idx=None, dim_tokens=None,
        ...
        self.gaussian_dim = 56
        self.gaussian_mode = None
        # self.head_gaussian_features = Mlp(in_features=idim,
        #                                hidden_features=int(hidden_dim_factor * idim),
        #                                out_features=56 * self.patch_size**2)
    ...
    def forward(self, decout, img_shape):
        ...
        # gaussian_features = self.head_gaussian_features(cat_output)  # B,S,D
