...
from .utils.geometry import inv, geotrf, normalize_pointcloud
from .utils.geometry import get_joint_pointcloud_depth, get_joint_pointcloud_center_scale
...
class Regr3D (Criterion, MultiLoss):
    ...
    def get_all_pts3d(self, batch, pred1, pred2, dist_clip=None):
        # everything is normalized w.r.t. camera of view1
        in_camera1 = inv(batch['context']['extrinsics_gt'][0]) #! world2cam
        gt_pts1 = geotrf(in_camera1[0:1], batch['context']['pts3d'][0][0:1])  # B,H,W,3
        gt_pts2 = geotrf(in_camera1[0:1], batch['context']['pts3d'][0][1:2])  # B,H,W,3 #* 두 view 다 canonical camera coordinate으로 변환

        valid1 = batch['context']['valid_mask'][0][0:1].bool().clone()
        valid2 = batch['context']['valid_mask'][0][1:2].bool().clone()
        ...
        pr_pts1 = get_pred_pts3d(batch, pred1, use_pose=False, view=0)
        pr_pts2 = get_pred_pts3d(batch, pred2, use_pose=True, view=1)
    ...
    def compute_loss(self, batch, pred1, pred2, **kw):
        gt_pts1, gt_pts2, pred_pts1, pred_pts2, mask1, mask2, monitoring = \
            self.get_all_pts3d(batch, pred1, pred2, **kw)
...
class ConfLoss (MultiLoss):
    ...
    def compute_loss(self, batch, pred1, pred2, **kw):
        ...
        ((loss1, msk1), (loss2, msk2)), details = self.pixel_loss(batch, pred1, pred2, **kw)
        if loss1.numel() == 0:
