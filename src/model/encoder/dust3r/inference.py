...
def loss_of_one_batch(batch, model, criterion, device, symmetrize_batch=False, use_amp=False, ret=None):
    ...
    with torch.cuda.amp.autocast(enabled=bool(use_amp)):
        pred1, pred2, dec1, dec2, feat1, feat2 = model(view1, view2)

        # loss is supposed to be symmetric
        with torch.cuda.amp.autocast(enabled=False):
            loss = criterion(view1, view2, pred1, pred2) if criterion is not None else None

    result = dict(view1=view1, view2=view2, pred1=pred1, pred2=pred2, loss=loss, dec1=dec1, dec2=dec2, feat1=feat1, feat2=feat2)
...
def get_pred_pts3d(batch, pred, use_pose=False, view=0):
    if 'depth' in pred and 'pseudo_focal' in pred:
        try:
            pp = batch['context']['intrinsics'][..., :2, 2]
        except KeyError:
            pp = None
