...
def depthmap_to_canonical_camera_coordinates(depthmap, camera_intrinsics, source_camera_pose, canonical_camera_pose, **kw):
    """
    Transform points from the depthmap in source camera coordinates to canonical camera coordinates.

    Args:
        - depthmap (HxW array): The depth values for each pixel in the source camera.
        - camera_intrinsics (3x3 array): Intrinsic parameters of the camera.
        - source_camera_pose (4x3 or 4x4 array): Source camera pose (camera to world transformation).
        - canonical_camera_pose (4x3 or 4x4 array): Canonical camera pose (camera to world transformation).

    Returns:
        - pointmap of canonical coordinates (HxWx3 array): 3D point coordinates in the canonical camera frame.
        - valid_mask (HxW array): Mask indicating which pixels have valid depth measurements.
    """
    # Convert depth map to camera coordinates
    X_cam, valid_mask = depthmap_to_camera_coordinates(depthmap, camera_intrinsics)

    # Extract rotation and translation from source camera pose
    R_src_cam2world = source_camera_pose[:3, :3]
    t_src_cam2world = source_camera_pose[:3, 3]

    # Extract rotation and translation from canonical camera pose
    R_canonical_cam2world = canonical_camera_pose[:3, :3]
    t_canonical_cam2world = canonical_camera_pose[:3, 3]

    # Transform source camera coordinates to world coordinates
    X_world_cam = np.einsum("ik, vjk -> vji", R_src_cam2world, X_cam) + t_src_cam2world

    # Transform world coordinates to canonical camera coordinates
    X_canonical_cam = np.einsum("ik, vjk -> vji", R_canonical_cam2world.T, X_world_cam) - t_canonical_cam2world

    return X_canonical_cam, valid_mask
...
def normalize_pointcloud_with_scale(pts1, pts2, norm_mode='avg_dis', valid1=None, valid2=None):
    """ renorm pointmaps pts1, pts2 with norm_mode
    """
    assert pts1.ndim >= 3 and pts1.shape[-1] == 3
    assert pts2 is None or (pts2.ndim >= 3 and pts2.shape[-1] == 3)
...
def create_transformation_matrix(norm_factor):
    """
    Create a 4x4 transformation matrix for point cloud normalization.
    :param norm_factor: Normalization factor as a PyTorch tensor.
    :return: 4x4 transformation matrix as a PyTorch tensor.
    """
    # Ensure norm_factor is at least 1D tensor for broadcasting
    if norm_factor.ndim == 0:
        norm_factor = norm_factor.unsqueeze(0)

    # Create the diagonal scaling matrix
    scale_matrix = torch.diag_embed(1.0 / norm_factor)

    # Initialize a 4x4 identity matrix
    transformation_matrix = torch.eye(4, dtype=scale_matrix.dtype, device=scale_matrix.device)

    # Embed the 3x3 scale matrix into the top-left corner of the 4x4 matrix
    transformation_matrix[:3, :3] = scale_matrix

    return transformation_matrix

# Example usage:
norm_factor = torch.tensor([0.5])  # Example normalization factor
trans_matrix = create_transformation_matrix(norm_factor)
print(trans_matrix)
