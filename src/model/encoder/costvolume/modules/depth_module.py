...
from src.geometry.projection import unproject, unproject_given_homogeneous
...
class BackprojectDepth(nn.Module):
    ...
    def forward(self, depth, K):
        K_repeat = repeat(K[:, 0:1, :3, :3], "b 1 i j -> b n i j", n=self.pix_coords.shape[0])
        K_repeat = rearrange(K_repeat, "b n i j -> (b n) i j")
        
        directions = unproject_given_homogeneous(
            rearrange(self.pix_coords, "b xyz n -> b n xyz"),
            K_repeat,
        )
        directions = directions / directions.norm(dim=-1, keepdim=True)
        
        cam_points = directions * rearrange(depth, "b () n () d -> b n d")
        self.ones = rearrange(self.ones, "b d n -> b n d")
