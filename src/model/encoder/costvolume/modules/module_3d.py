...
def homo_warp_cas(src_feat, proj_mat, depth_values):
    """
    src_feat: (B, C, H, W)
...
def homo_warping_trans(src_fea, src_proj, ref_proj, depth_values):
    # src_fea: [B, C, H, W]
    # src_proj: [B, 4, 4]
    # ref_proj: [B, 4, 4]
...
class CostRegNetWeight(nn.Module):
    def __init__(self, in_channels, base_channels):
        super(CostRegNetWeight, self).__init__()
        self.conv0 = nn.Conv3d(in_channels, base_channels, kernel_size=3, padding=1)

        self.conv1 = nn.Conv3d(base_channels, base_channels * 2, kernel_size=3, stride=2, padding=1)
        self.conv2 = nn.Conv3d(base_channels * 2, base_channels * 2, kernel_size=3, padding=1)

        self.conv3 = nn.Conv3d(base_channels * 2, base_channels * 4, kernel_size=3, stride=2, padding=1)
        self.conv4 = nn.Conv3d(base_channels * 4, base_channels * 4, kernel_size=3, padding=1)

        self.conv5 = nn.Conv3d(base_channels * 4, base_channels * 8, kernel_size=3, stride=2, padding=1)
        self.conv6 = nn.Conv3d(base_channels * 8, base_channels * 8, kernel_size=3, padding=1)

        self.conv7 = nn.ConvTranspose3d(base_channels * 8, base_channels * 4, kernel_size=3, stride=2, padding=1, output_padding=1)

        self.conv9 = nn.ConvTranspose3d(base_channels * 4, base_channels * 2, kernel_size=3, stride=2, padding=1, output_padding=1)

        self.conv11 = nn.ConvTranspose3d(base_channels * 2, base_channels, kernel_size=3, stride=2, padding=1, output_padding=1)

        # Last layer for 8-dim feature volume
        self.features = nn.Conv3d(base_channels, base_channels, kernel_size=3, stride=2, padding=1, bias=False)
        self.featues_2 = nn.Conv3d(base_channels, 128, kernel_size=3, stride=2, padding=1, bias=False)

        # Additional layer for adaptive weight volume with a single channel
        self.weights = nn.Conv3d(base_channels, 1, kernel_size=3, stride=2, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        # Compute 8-dim feature volume
        x = self.features(x)
        features = self.featues_2(x)
        # Compute adaptive weight volume and apply sigmoid activation function
        weights = torch.sigmoid(self.weights(x))
