...
class TransMVSNet(nn.Module):
    ...
    def forward(self, imgs, proj_matrices, depth_values, test_tnt=False, mvs_volume_only=False):
        ...
        for stage_idx in range(self.num_stage):
            ...
            near_far = depth_range_samples[:, [0, -1]]
            ...
            var =  (outputs_stage['prob_volume'] * (outputs_stage['depth_values'] - depth.unsqueeze(1))**2).sum(1)
            std = torch.clamp_min(var, 1e-10).sqrt()
            
            outputs_stage['depth'] = depth
            outputs_stage['std'] = std
            outputs_stage['prob_volume'] = outputs_stage['prob_volume']
        ...
        return features_backbone, outputs, near_far
