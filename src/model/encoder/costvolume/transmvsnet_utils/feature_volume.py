...
from .cnn3d import VolumeRegularization
from .fmt.module import CostRegNetWeight
...
class FeatureVolume(nn.Module):
    ...
    def forward(self, feats, batch):
        """
        feats: [B NV C H W], NV: number of views (source features)
...
class MVSVolume(nn.Module):
    """
    Create the coarse feature volume in a C2F2NeuS-way
    """
    def __init__(self, in_channels=[48, 32, 8], base_channels=8):
        """
        Set up the volume grid given resolution
        """
        super().__init__()
        
        # self.cost_reg_2 = CostRegNetWeight(in_channels=in_channels, base_channels=base_channels)
        
        for i, ch in enumerate(in_channels):
            setattr(self, f"unet_{i+1}", Unet(in_channels=ch, base_channels=base_channels, out_channels=base_channels))
        
    def forward(self, batch, volume, stage=1):
        """
        feats: [B NV C H W], NV: number of views (source features)
        batch: to get the poses for homography
        """
        
        feat = volume.squeeze(1)
        feat = getattr(self, f"unet_{stage}")(feat)
        feat, weight = feat[:, :-1], feat[:, -1:]
        # make the channel dimension 1 to make it work as the feature weight
