...
def poisson_mesh(path, vtx, normal, color, depth, thrsh):
    ...
    ms.generate_surface_reconstruction_screened_poisson(depth=depth, preclean=True, samplespernode=1.5) #! killed 유발하는 후보
    ...
    nn_dist = nn_dist[0]
    nn_idx = nn_idx[0]
    nn_color = torch.mean(color.cuda()[nn_idx], axis=1)
