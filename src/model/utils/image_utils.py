...
from ...geometry.projection import sample_image_grid, unproject, homogenize_vectors, transform_cam2world
...
def depth2normal(depth, mask, intr, image_size=(224, 224)):
    """
    Computes surface normals from a depth map using intrinsic camera parameters.

    Args:
    - depth (Tensor): The depth map tensor.
    - mask (Tensor): A mask indicating valid regions in the depth map.
    - intr (Tensor): The camera's intrinsic matrix.
    - image_size (tuple): The size of the image (height, width).

    Returns:
    - Tensor: Normalized surface normals for each pixel.
    """

    # Convert depth to camera space coordinates
    camD = depth.permute([1, 2, 0])
    mask = mask.permute([1, 2, 0])
    h, w = torch.meshgrid(torch.arange(camD.shape[0]), torch.arange(camD.shape[1]), indexing='ij')
    h = h.to(depth.device).to(torch.float32)
    w = w.to(depth.device).to(torch.float32)
    p = torch.stack([w, h], dim=-1)

    # Adjust points by principal point
    principal_point = intr[0:2, 2]  # cx, cy
    p[..., 0] -= principal_point[0] * image_size[1]
    p[..., 1] -= principal_point[1] * image_size[0]
    p *= camD

    # Inverse of the intrinsic matrix's top-left 2x2 block for point scaling
    K = intr[:2, :2]
    Kinv = torch.inverse(K).to(depth.device)
    camPos = torch.cat([p @ Kinv.t(), camD], dim=-1)

    # Pad camera position and mask to handle edge cases
    ...
    # Calculate difference vectors and cross products
    p_c = camPos[:, 1:-1, 1:-1]
    p_u = camPos[:, :-2, 1:-1] - p_c
    p_l = camPos[:, 1:-1, :-2] - p_c
    p_b = camPos[:, 2:, 1:-1] - p_c
    p_r = camPos[:, 1:-1, 2:] - p_c

    # Compute normals using cross products between neighboring difference vectors
    ...
    n = torch.nn.functional.normalize(n[0], dim=-1)
    # Apply the mask and permute dimensions to (channels, height, width)
    n = (n * mask[0, 1:-1, 1:-1]).permute([2, 0, 1])
...
def depth2wpos(depth, mask, intrinsic, extrinsic, idx=0):
    camD = depth.permute([1, 2, 0])
    mask = mask.permute([1, 2, 0])
    shape = camD.shape
    device = camD.device
    ...
    p, _ = sample_image_grid((h, w), device)
    p = rearrange(p, "h w xy -> (h w) () xy")
    camD = rearrange(camD, "h w d -> (h w) () d")[..., 0]
    # unproject to get camera-space coordinates
    
    intrinsic = intrinsic[None, None, None, ...]
    camPos = unproject(p, camD, intrinsic)
    
    # homogenize_vectors
    extrinsic = extrinsic[None, None, None, ...]
    camPos = homogenize_vectors(camPos)
    camWPos = transform_cam2world(camPos, extrinsic)[..., :-1]
    
    camWPos = rearrange(camWPos, "() (h w) () xyz -> h w xyz", h=h, w=w)
    
    camWPos = (camWPos * mask).permute([2, 0, 1])
    
    
    
    
    # K00 = intrinsic[0, 0] * image_width
    # K11 = intrinsic[1, 1] * image_height #! 차이 나는 부분 gaussian surfels에서는 반대로 함.
    ...
    # world_view_transform = c2w.inverse()
    # pose = world_view_transform
    # Rinv = pose[:3, :3]
    # t = pose[3:, :3]
...
def resample_points(intrinsic, extrinsic, depth, normal, color, mask, image_size=(224, 224)):
    
    camWPos = depth2wpos(depth, mask, intrinsic, extrinsic).permute([1, 2, 0])
    ...
    camworld_view_transform = extrinsic.inverse()
    Rinv = camworld_view_transform[:3, :3]
