...
def rays_to_cameras(
    ...
    if cameras is None:
        I_camera = PerspectiveCameras(focal_length=focal_lengths.expand(directions.shape[0], -1), principal_point=principal_points.expand(directions.shape[0], -1), device=device).clone()
...
def compute_ndc_coordinates(
    ...
    if crop_parameters is None:
        ...
        device = crop_parameters.device if crop_parameters is not None else device
